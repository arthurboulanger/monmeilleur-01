...
class CustomSidebars{
	...
	function addStyles($hook){
        if( 'widgets.php' == $hook || 'appearance_page_customsidebars' == $hook){
        	wp_enqueue_script( 'cs_script', get_template_directory_uri() . '/inc/custom-sidebars/cs.js', array(), '1.0.0', true );
        	//wp_enqueue_script('thickbox', null, array('jquery'));
        	//wp_enqueue_style('thickbox.css', includes_url() . 'js/thickbox/thickbox.css', null, '1.0');
        }
	    wp_enqueue_style( 'cs_style', get_template_directory_uri() . '/inc/custom-sidebars/cs_style.css' );
	}
	...
	function getEmptyWidget(){
		return array(
			...
			'id' => 'csemptywidget',
			'callback' => array(new CustomEmptySidebars(), 'display_callback'),
			'params' => array(array('number' => 2)),
			'classname' => 'CustomEmptySidebars',
...
if(!isset($blogy_unl_sidebars)){
	$blogy_unl_sidebars = new CustomSidebars();
	add_action( 'widgets_init', array($blogy_unl_sidebars,'registerCustomSidebars') );
	add_action( 'widgets_admin_page', array($blogy_unl_sidebars,'widgetSidebarContent'));
	add_action( 'save_post', array($blogy_unl_sidebars,'storeReplacements'));
	add_action( 'admin_enqueue_scripts', array($blogy_unl_sidebars,'addStyles'));

	//Frontend
	add_action( 'wp_head', array($blogy_unl_sidebars,'replaceSidebars'));
	add_action( 'wp', array($blogy_unl_sidebars, 'storeOriginalPostId'));

    //AJAX actions
    add_action( 'wp_ajax_cs-ajax', array($blogy_unl_sidebars, 'ajaxHandler'));

    add_filter('admin_body_class', array($blogy_unl_sidebars, 'checkMP6'));

}
if(! class_exists('CustomEmptySidebars')){
class CustomEmptySidebars extends WP_Widget {
	function CustomEmptySidebars() {
		parent::WP_Widget(false, $name = 'CustomEmptySidebars');
	}
